...
class RRTStar(RRT):
    def __init__(self, grid, obstacle_threshold=200, max_iter=2500, step_size=2, search_radius_factor=1.0):
        super().__init__(grid, obstacle_threshold, max_iter, step_size)
    ...
    def rewire(self, new_node):
        radius = self.search_radius_factor * np.sqrt((np.log(len(self.nodes)) / len(self.nodes)))
        radius = max(self.step_size, radius)
        for node in self.nodes:
            if node == new_node or not self.is_valid_node(node):
                continue
            if self.distance(node, new_node) <= radius + self.step_size:
                temp_parent = new_node.parent
                new_node.parent = node
                if self.cost(new_node) < self.cost(temp_parent):
                    continue  # If the cost is less with the new parent, keep it
                else:
                    new_node.parent = temp_parent  # If not, revert to the original parent
