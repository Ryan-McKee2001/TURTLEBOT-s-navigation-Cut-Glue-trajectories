...
class TestRRTIntegration(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        grid_df = pd.read_csv(os.getcwd() + '\\rrt\\tests\\test_grids\\tb3_world_costmap.csv', header=None)
        grid = grid_df.values.tolist()
        cls.planner = RRT(grid)
    ...
    def test_rrt_shortest_path(self, start_coords, end_coords):
        self.start_coords = start_coords
        self.end_coords = end_coords
        print (f"Testing path from {start_coords} to {end_coords} on TB3 world costmap")
        start_time = time.time()
        path, cost = self.planner.find_path(start_coords, end_coords) # , cost = self.planner.find_path(start_coords, end_coords)
        end_time = time.time()
        print (f"Path: {path}")
        print (f"Cost: {cost}")
    ...
    def test_rrt_with_exception_inputs(self):
